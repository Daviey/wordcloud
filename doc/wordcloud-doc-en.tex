...
\parbox{0.6\textwidth}{
\wordcloud[scale=2,rotate=0,horizontal_only,margin=0.5pt,usecolor]{(Wordcloud,10);(\hologo{METAPOST},6);(\LaTeX,7);(Lua,4);(algorithm,3);(code,2);(mathematics,2);(CTAN,2);(mplib,4);(\hologo{LuaTeX},4);(latexmp,3);(graphism,2)}
}\hfill
...
\begin{description}
  ...
  \item[\texttt{aspect=}\meta{number}:] to adjust the placement aspect ratio.
  Values $>1$ tend to produce a taller cloud; values $<1$ tend to produce a
  wider cloud (default 1) ;
  \item[\texttt{fit\_width=}\meta{dimension}:] to scale the final output so it
  fits within the given width (default \lstinline+0pt+, disabled) ;
  \item[\texttt{fit\_height=}\meta{dimension}:] to scale the final output so it
  fits within the given height (default \lstinline+0pt+, disabled) ;
  \item[\texttt{horizontal\_only}:] to force all words to be horizontal (boolean,
  default \lstinline+false+). When enabled, per-word alternating rotation is
  disabled ;
